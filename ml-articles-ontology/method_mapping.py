...
left = pd.read_csv("article_methods.csv")   # ml_method,label  (mla-uri)
right = pd.read_csv("ml_methods.csv")       # ml_method,label  (ml-uri)
...
with open("method_mapping.ttl", "w", encoding="utf-8") as f:
    f.write("\n".join(ttl_lines) + "\n")
...
unmapped.to_csv("unmapped_methods.csv", index=False)
